    ...
    jQuery('.datepicker-holder').each(function () {
        ...
        inputs.datepicker({
            ...
            onSelect: function (selectedDate) {
                ...
                if (input.is(from)) {
                    //abe++
                    if (to.attr('type') === 'text') {//Days are displayed range mode (cocorico_time.days_display_mode: range)
                        setTimeout(function () {
                            to.focus();
                        }, 100);
...
function syncTimeFields(parentTimesElt) {
    $(parentTimesElt).each(function () {
        ...
        //Times are displayed in duration mode (cocorico_time.times_display_mode: duration)
